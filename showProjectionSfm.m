%%
function showProjectionSfm(ply_filename, poseDir)
close all;
clear;

if (nargin < 2)
    ply_filename_my = 'E:/tools/pose_from_contour/data/mydata/testModel.ply';
ply_filename = 'E:/RealFootData/data_new/video3/visualSFMResult/visualSFM.onlyfoot.ply';
poseDir = 'E:/RealFootData/data_new/video3/visualSFMResult/visualSFM.nvm.cmvs/00';
savepath = 'E:/RealFootData/data_new/video3/mask/';
end
...
[node_xyz_my, node_rgb_my] = ply_to_points(ply_filename_my);
...
if DrawAll
    ...
    hold on;
    scatter3(node_xyz(1,:), node_xyz(2,:), node_xyz(3,:), '.', 'cdata', node_rgb'./255.0);
    scatter3(node_xyz_my(1,:), node_xyz_my(2,:), node_xyz_my(3,:), '.');
    hold off;
end
...
if (any(strcmp(Names, 'red')) & any(strcmp(Names, 'green')) & any(strcmp(Names, 'blue')) )
    eval(['node_rgb = [Elements.', Name, '.red, Elements.', Name, '.green, Elements.', Name, '.blue];']);
else
    node_rgb = zeros(1, 3);
end
node_rgb = node_rgb';
